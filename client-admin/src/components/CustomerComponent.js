...
class Customer extends Component {
  ...
  render() {
    const customers = this.state.customers.map((item) => {
      return (
        <tr key={item._id} className="datatable" onClick={() => this.trCustomerClick(item)}>
          <td>{item._id}</td>
          <td><img src={"data:image/jpg;base64," + item.profile} width="70px" height="70px" alt="" /></td>
          <td>{item.username}</td>
          <td>{item.password}</td>
          <td>{item.name}</td>
          <td>{item.phone}</td>
    ...
    return (
      <div>
      <div class='product_containtAd'>
        <div className="align-center" class='product_com'>
          <h2 className="text-center">CUSTOMER LIST</h2>
          <table className="datatable" border="1">
            <tbody>
              <tr className="datatable" class='data_P'>
                <th>ID</th>
                <th>Avatar</th>
                <th>Username</th>
                <th>Password</th>
